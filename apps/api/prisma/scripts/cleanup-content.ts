/**
 * Safe tenant content cleanup — keeps only the preserved master user and platform config.
 * Does NOT re-seed demo users or events. Preserved email: felipe.e.salom@gmail.com
 *
 * Usage:
 *   pnpm db:cleanup-content              # dry-run (default)
 *   pnpm db:cleanup-content -- --confirm
 *   pnpm db:cleanup-content -- --confirm --include-subcategories
...
function assertSafeEnvironment() {
  if (process.env.NODE_ENV === 'production' && process.env.ALLOW_PRODUCTION_CLEANUP !== 'true') {
    ...
    console.error(
      'To override (not recommended): ALLOW_PRODUCTION_CLEANUP=true pnpm db:cleanup-content -- --confirm',
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  console.log('Yo Te Invito — content cleanup (preserves master user only)');
  ...
  if (dryRun) {
    ...
    console.log('  pnpm db:cleanup-content -- --confirm');
    return;
  }
