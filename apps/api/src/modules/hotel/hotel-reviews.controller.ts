...
export class HotelReviewsController {
  ...
  getSummary(@CurrentUser() user: { id: string; tenantId: string; role: string }) {
    return this.reviewDisputes.getHotelSummary(user.tenantId, user.id, user.role);
  }
  ...
  listReviews(
    ...
    return this.reviewDisputes.listHotelReviews(user.tenantId, user.id, user.role, query);
  }
