...
export function renderReviewDisputeAccepted(
  ...
  const subject = 'La disputa de reseña fue aceptada';
  const previewText =
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 12px;">Administración revisó la disputa vinculada a <strong>${escapeHtml(entityTitle)}</strong> y decidió <strong>aceptarla</strong>.</p>
    <p style="margin:0 0 12px;">Esto puede implicar una acción de moderación sobre la reseña, según corresponda al caso.</p>
    ${noteBlock}
  `;
  ...
  const text = [
    `Hola ${recipientName},`,
    '',
    `Disputa aceptada para ${entityTitle}.`,
    resolutionNote ? `\n${resolutionNote}` : '',
    '',
