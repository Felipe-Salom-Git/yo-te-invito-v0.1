  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...body, role: 'HOTEL_OWNER', tenantId: 'tenant-demo' }),
      });
      ...
      router.push('/register/hotel/success');
      router.refresh();
    } catch (err) {
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-8 p-8">
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          <h1 className="text-xl font-semibold text-text">Solicitar cuenta hotel</h1>
          <p className="text-sm text-text-muted">
            Completá el formulario. Un administrador revisará tu solicitud.
          </p>
        </CardHeader>
        <CardContent>
