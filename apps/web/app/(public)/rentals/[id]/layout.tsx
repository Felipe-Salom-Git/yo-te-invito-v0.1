...
async function RentalJsonLd({ id }: { id: string }) {
  const event = await fetchPublicEvent(id);
  if (!event) return null;

  const title = typeof event?.title === 'string' && event.title.trim() ? event.title : 'Rental';
  const jsonLd = buildEventJsonLd({
    url: `${process.env.NEXT_PUBLIC_APP_URL ?? 'https://yoteinvito.club'}/rentals/${encodeURIComponent(id)}`,
    name: title,
