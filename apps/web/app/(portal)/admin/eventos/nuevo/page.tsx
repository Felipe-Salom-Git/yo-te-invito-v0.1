...
export default function AdminEventoNuevoPage() {
  ...
  return (
    <PageContainer>
      <Link href="/admin/eventos" className="mb-4 inline-block text-sm text-text-muted hover:text-text">
        ← Eventos
      </Link>
      <SectionTitle>Crear evento promocional</SectionTitle>
      <p className="mt-2 text-text-muted">
        Eventos sin venta de tickets (promocionales, difusión, etc.)
      </p>

      <form onSubmit={handleSubmit} className="mt-6 max-w-xl space-y-4">
        <Input label="Título" value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="Nombre del evento" />
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded border border-border bg-bg px-3 py-2 text-text placeholder:text-text-muted"
            placeholder="Descripción opcional"
          />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Input label="Ciudad" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Buenos Aires" />
          <Input label="Lugar" value={venueName} onChange={(e) => setVenueName(e.target.value)} placeholder="Teatro / Sala" />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Input label="Fecha inicio (opcional)" type="datetime-local" value={startAt} onChange={(e) => setStartAt(e.target.value)} />
          <Input label="Fecha fin (opcional)" type="datetime-local" value={endAt} onChange={(e) => setEndAt(e.target.value)} />
        </div>
        <Input label="Capacidad (opcional)" type="number" value={capacityTotal} onChange={(e) => setCapacityTotal(e.target.value)} placeholder="Ej. 100" />
        <Input label="Valor (opcional)" type="number" value={valueOptional} onChange={(e) => setValueOptional(e.target.value)} placeholder="Precio base" />
        <div>
          <label className="mb-1.5 block text-sm font-medium text-text">Imagen</label>
          <Input label="URL de imagen" value={coverImageUrl} onChange={(e) => setCoverImageUrl(e.target.value)} placeholder="https://…" />
        ...
        <div>
          ...
          <Input label="Dirección" value={venueAddress} onChange={(e) => setVenueAddress(e.target.value)} placeholder="Calle, número, ciudad" />
