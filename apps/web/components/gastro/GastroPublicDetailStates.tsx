...
export function GastroPublicDetailLoading() {
  return (
    <div className="min-h-screen bg-bg">
      <div className="animate-pulse">
        ...
        <div className="mx-auto max-w-7xl space-y-4 px-4 py-8 sm:px-6 md:px-8">
          ...
          <div className="h-4 w-full max-w-2xl rounded bg-bg-muted" />
...
export function GastroPublicDetailError() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-bg px-6 py-16 text-center">
      <p className="text-lg font-semibold text-text">Local no encontrado</p>
      <p className="mt-2 max-w-md text-sm text-text-muted">
        El restaurante no está disponible o no tiene ficha pública activa.
      </p>
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        <Link
          href="/categoria/gastro"
          className="rounded-lg bg-accent px-4 py-2 text-sm font-medium text-bg hover:opacity-90"
        >
          Ver gastronomía
        </Link>
        <Link href="/home" className="rounded-lg border border-border px-4 py-2 text-sm text-text hover:border-accent/50">
          Ir al inicio
        </Link>
      </div>
    </div>
