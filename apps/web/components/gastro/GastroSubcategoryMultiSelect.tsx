...
export type GastroSubcategoryMultiSelectProps = {
  value: string[];
...
/** Gastro multi-select. First selected chip is the primary (`GastroProfile.subcategoryId`). */
export function GastroSubcategoryMultiSelect({
  value,
  onChange,
  disabled = false,
  className = '',
}: GastroSubcategoryMultiSelectProps) {
  const { data: items = [], isLoading } = usePublicSubcategories('gastro');
  ...
  if (items.length === 0) {
    return (
      <p className={`text-sm text-text-muted ${className}`.trim()}>
        Todavía no hay subcategorías disponibles para gastronomía.
      </p>
    );
  }
  ...
  return (
    <div className={`space-y-3 ${className}`.trim()}>
      <div>
        <label className="block text-sm font-medium text-text-muted">Subcategorías gastronómicas</label>
...
export function gastroSubcategoryIdsToPayload(ids: string[]): {
...
export function gastroSubcategoryIdsFromLocal(local: {
  subcategoryId?: string | null;
  subcategories?: Array<{ id: string }> | null;
}): string[] {
  if (local.subcategories?.length) {
    return local.subcategories.map((s) => s.id);
  }
  return local.subcategoryId ? [local.subcategoryId] : [];
}
