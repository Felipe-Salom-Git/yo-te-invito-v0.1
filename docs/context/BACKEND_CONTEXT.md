...
- **Scanner ownership (V3.1 Etapa 5.1):** modelo `ScannerAccount` — usuario `SCANNER` vinculado a un perfil comercial (`PRODUCER`, `GASTRO`, `EXCURSION_OPERATOR`, `RENTAL_LOCATION`) sin membership de portal. Endpoints: `GET /producer/scanners`, `GET /gastro/scanners`, `GET /admin/scanner-accounts`, `GET /scanner/account`. Servicio: `ScannerAccountsService`. Smoke: `pnpm --filter api run smoke:v31-scanner-accounts`. Doc: `docs/audits/V3_1_STAGE_5_SCANNER_ACCOUNTS_SMOKE.md`.
