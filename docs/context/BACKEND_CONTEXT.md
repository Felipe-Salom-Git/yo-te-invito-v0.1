...
- **V3.3 Etapa 9 — Liquidaciones, Transferencias y Canjes (cerrada 2026-08-31):** **manual financial settlement only** — sin pasarela, sin API bancaria, sin facturación. **Prisma:** `BenefitCommercialAgreement` (GASTRO→`gastroProfileId` XOR ACTIVITY→`excursionOperatorId`; `unitPriceCents` BigInt; `barterMultiplier` Decimal; `validFrom`/`validTo` inclusive AR); `BenefitSettlement` (unique parcial tenant+vertical+partner+`periodKey`); `BenefitSettlementUsageAllocation` (`CASH`|`BARTER`; snapshot agreement; `@@unique([validationSource, validationId])`); `BenefitSettlementTransfer` (append-only, reversal fields); `CourtesyCreditLedgerEntry` (append-only; `sourceAllocationId`/`sourceCourtesyCampaignId` unique). **Services:** `BenefitCommercialAgreementsService`, `BenefitSettlementsService` (generate/refresh/allocate/close; allocate `Serializable`+`FOR UPDATE`), `BenefitSettlementTransfersService`, `CourtesyCreditLedgerService`, `BenefitSettlementReportingService`, `BenefitSettlementIntegrityService`. **Controllers:** `/admin/benefit-agreements`, `/admin/benefit-settlements`, `/admin/benefit-transfers`, `/admin/courtesy-credits`, `/admin/benefit-reporting` — ADMIN only; tenant-scoped. **Eligibility:** Gastro `claimId != null`; Activity `result=VALID` + `claimId`; master QR excluido. **CASH:** due/received/outstanding derivados; overpayment rechazado. **BARTER:** `roundBarterCreditCents` half-up; materialización idempotente en misma tx que allocation. **Gastro funding:** `POST /admin/gastronomicos/:profileId/courtesy` con debit ledger; GASTRO_OWNER cortesía histórica sin funding intacta. **Delete safety:** `countPartnerSettlementAllocations` en deep delete/purge gastro+excursion; integrity orphan/partner mismatch. **Scripts:** `benefit-settlements:materialize-barter-credits` (idempotente), `benefit-settlements:audit-integrity` (read-only, tenant-safe). Migraciones `20260901120000`…`20260901200000` (**no aplicadas localmente** — P1001). Tests PASS: `test:benefit-commercial-agreements`, `test:benefit-settlement-domain`, `test:benefit-settlement-transfers`, `test:courtesy-credit-ledger`, `test:gastro-courtesy-credit-funding`, `test:benefit-settlement-reporting`, `test:benefit-settlement-hardening`, `test:benefit-settlement-admin-ui`. Docs: `V3_3_STAGE_9_SETTLEMENTS_AUDIT.md`, `V3_3_STAGE_9_SETTLEMENTS_CLOSING.md`. Hardening `b920553`.
...
- **BenefitCommercialAgreement**, **BenefitSettlement**, **BenefitSettlementUsageAllocation**, **BenefitSettlementTransfer**, **CourtesyCreditLedgerEntry** (Etapa 9): settlement manual Gastro/Activity; money `BigInt` cents; allocation snapshot histórico; `validationSource`+`validationId` lógico (sin FK); transfer/ledger append-only; `agreementId` FK Restrict; actor FKs `SetNull`.
...
| Test benefit commercial agreements | `pnpm --filter api run test:benefit-commercial-agreements` |
| Test benefit settlement domain | `pnpm --filter api run test:benefit-settlement-domain` |
| Test benefit settlement transfers | `pnpm --filter api run test:benefit-settlement-transfers` |
| Test courtesy credit ledger | `pnpm --filter api run test:courtesy-credit-ledger` |
| Test gastro courtesy credit funding | `pnpm --filter api run test:gastro-courtesy-credit-funding` |
| Test benefit settlement reporting | `pnpm --filter api run test:benefit-settlement-reporting` |
| Test benefit settlement hardening | `pnpm --filter api run test:benefit-settlement-hardening` |
| Test benefit settlement admin UI helpers | `pnpm --filter api run test:benefit-settlement-admin-ui` |
| Audit integrity settlements (read-only DB) | `pnpm --filter api run benefit-settlements:audit-integrity` |
| Materialize barter credits (idempotent DB) | `pnpm --filter api run benefit-settlements:materialize-barter-credits` |
