...
- **Scanner short codes (V3.3 Etapa 3):** `ScannerShortCodeService` — tickets vía `shortTicketCode` (8 chars, scope evento); gastro vía `GastroDiscountClaim.shortCode` (6 chars, lookup → `buildGastroDiscountQrPayload`); util `manual-short-code.util.ts`. Regla: short code ≠ token QR. Migración `20260831120000_gastro_claim_short_code` (+ `CREATE EXTENSION IF NOT EXISTS pgcrypto`).
...
- **GastroDiscount**, **GastroDiscountValidation**, **InboxItem** (kind incl. `REVIEW_DISPUTE_REQUEST`). **Etapa 5:** `pendingUpdate` JSON, `archivedAt`, `createdByOrigin` (`GASTRO`\|`ADMIN`), `createdByUserId`; FKs creator/submitter `onDelete: SetNull`.
