...
- **Scanner ownership (V3.1 Etapa 5):** modelo `ScannerAccount` — usuario `SCANNER` vinculado a un perfil comercial (`PRODUCER`, `GASTRO`, `EXCURSION_OPERATOR`, `RENTAL_LOCATION`) sin membership de portal. **5.1:** listados + link admin + `GET /scanner/account`. **5.2:** `POST/PATCH/reset-password` en `/producer/scanners` y `/gastro/scanners` (crear usuario, activar/desactivar, reset clave). Servicio: `ScannerAccountsService`; util `password.util.ts`. Smoke: `pnpm --filter api run smoke:v31-scanner-accounts`. Docs: `docs/audits/V3_1_STAGE_5_SCANNER_ACCOUNTS_SMOKE.md`, `docs/audits/V3_1_STAGE_5_SCANNER_USERS_SMOKE.md`.
