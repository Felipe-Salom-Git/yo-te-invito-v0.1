...
- **Scanner ownership (V3.1 Etapa 5 — cerrada):** modelo `ScannerAccount`; portales gestionan usuarios; PWA en `apps/scanner` con cámara QR, manifest, selección scoped (`GET /scanner/scan-targets`); scope en `scan`/`tickets`/gastro validate vía `assertScannerCanAccess*`. Smokes: `smoke:v31-scanner-accounts`, `smoke:v31-scanner-scope`. Doc cierre: `docs/audits/V3_1_STAGE_5_CLOSING.md`.
