...
- Portal gastro: dashboard + validaciones (Slice 6). **Imágenes GCS:** `GastroLocalForm`, `GastroDiscountForm`, `/gastro/contenido`. **Contenido editorial (hotfix 2026-06-23):** `GASTRO_OWNER` no carga lista global de eventos gastro ni muestra select; usa `getMyLocal().publicEventId`. **Subcategorías múltiples (2026-06-23):** `GastroSubcategoryMultiSelect` + sync `EventSubcategory` en evento público. Valoraciones: `ManagedReviewsCommentsPage` scope `gastro` + `ManagedPortalReviewAlerts`. Follows: `GastroFollowButton`, `MePreferencesGastro` (toggles web/email por local). Notificaciones descuento: kind `FOLLOWED_GASTRO_NEW_DISCOUNT` en bandeja `/me/notifications`. **Scanner panel:** `ScannerUsersPanel` toast post-creación indica login inmediato en app scanner. **Admin dashboard:** sección «Pendientes operativos» (borradores + descuentos gastro).
...
- **VPS redirect smoke:** checkout app → Getnet hosted checkout OK; webhook portal URL + Basic Auth configurados; primer webhook rechazado por schema (fix `ed0cc3e` en repo) — [GETNET_WEBCHECKOUT_VPS_REDIRECT_SMOKE.md](../payments/GETNET_WEBCHECKOUT_VPS_REDIRECT_SMOKE.md), [GETNET_WEBHOOK.md](../payments/GETNET_WEBHOOK.md).
...
**Eventos multi-fecha (V3.1 Etapa 7 — cerrada 2026-06-10):** Productora — toggle simple/multi en wizard, `EventOccurrencesEditor`, `TicketTypesEditor` con tabs por fecha (`lib/producer/event-occurrences.ts`). Público — `EventDateSelector` en `/events/[eventId]`, `EventPurchaseCard` + `useAddToCart` con `occurrenceId`. Comprador — fecha de función en `MeTicketListCard` / `DefaultBuyerTicket`. Admin — badge «Multi-fecha» en listado. Scanner PWA — selector de fecha + UX `WRONG_OCCURRENCE` en `DoorScannerClient`. Doc: `docs/audits/V3_1_STAGE_7_MULTI_DATE_EVENTS_CLOSING.md`.

**Transferencia de entradas (V3.1 Etapa 9 — cerrada 2026-06-10):** `/me/tickets/[ticketId]` — `TicketTransferPanel`; `/me/ticket-transfer/[token]` — aceptar/rechazar; `/me/activity?tab=transfers` — `TransferOfferRow`; errores ES en `lib/me/ticket-transfer-labels.ts`. Doc: `docs/user/TICKET_TRANSFER.md`, cierre `V3_1_STAGE_9_TICKET_TRANSFER_CLOSING.md`.

**Cambio de fecha entrada (V3.1 Etapa 8 — cerrada 2026-06-10):** `/me/tickets/[ticketId]` — `TicketDateChangePanel` (opciones, solicitud, historial). Productora — `ProducerTicketDateChangesPanel` en `/producer/events/[eventId]`. Orden — badge «Fecha modificada» en `MeOrderTicketsList`. Notificaciones — kinds `TICKET_DATE_CHANGE_*` en `/me/notifications`. Doc: `docs/tickets/TICKET_DATE_CHANGE_POLICY.md`, cierre `V3_1_STAGE_8_TICKET_DATE_CHANGE_CLOSING.md`.
