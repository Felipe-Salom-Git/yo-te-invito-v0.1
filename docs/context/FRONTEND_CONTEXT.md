...
| Account | `/login`, **`/register`** (wizard `RegisterWizard`: cuenta → perfil → paso por tipo → legales SIGNUP → `POST /auth/register` sin auto-login → `/login?registered=1&verifyEmail=1`), **`/cuenta/solicitar-gastro`** (mismos campos mínimos + opcionales), **`/me/*`** (portal usuario estándar) |
