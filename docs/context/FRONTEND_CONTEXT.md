...
| Productora eventos | `ProducerEventCreateForm` / `ProducerEventEditForm` — wizard 3 pasos (`ProducerEventWizardProgress`, `wizardStep`); toggle simple/multi-fecha paso 2 + `EventOccurrencesEditor` (V3.1 Etapa 7); `ProducerEventPublicationLegalNotice` paso 3; ticket types post-create en `/producer/events/[id]` (tabs por fecha si multi-fecha) |
