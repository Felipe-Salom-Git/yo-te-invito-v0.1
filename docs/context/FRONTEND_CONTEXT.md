...
| Usuarios admin | `components/admin/users/` — `AdminUsersPageClient`, filtros URL, tabla + cards mobile, badges rol/perfiles; cambio de rol con confirmación; **eliminar usuario** (`AdminUserDeleteModal`, preflight + confirmación fuerte); **eliminación profunda** (`AdminDeepDeleteButton` → `AdminDeepDeleteModal`); cuenta maestro / self-delete sin acción; hooks `lib/query/admin-users.ts`, `lib/query/admin-deep-delete.ts` |
