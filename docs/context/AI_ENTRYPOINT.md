...
| **V3.3 funcional/operativa** | **Activa** — Etapa 0 auditoría cerrada; **Etapas 1–9** implementadas (UX pública/mobile, Avatar, Scanner V3, Gastro Multi-local + Approval, Descuentos Gastro V3, QR Studio, Actividades + Cupones QR, **Campañas Email / WhatsApp**, **Liquidaciones / Transferencias / Canjes**); QA manual/integración acumulado pendiente — [`V3_3_STAGE_1_PUBLIC_MOBILE_CLOSING.md`](../audits/V3_3_STAGE_1_PUBLIC_MOBILE_CLOSING.md), [`V3_3_STAGE_2_USER_AVATAR_CLOSING.md`](../audits/V3_3_STAGE_2_USER_AVATAR_CLOSING.md), [`V3_3_STAGE_3_SCANNER_V3_CLOSING.md`](../audits/V3_3_STAGE_3_SCANNER_V3_CLOSING.md), [`V3_3_STAGE_4_GASTRO_MULTI_LOCAL_CLOSING.md`](../audits/V3_3_STAGE_4_GASTRO_MULTI_LOCAL_CLOSING.md), [`V3_3_STAGE_5_GASTRO_DISCOUNTS_CLOSING.md`](../audits/V3_3_STAGE_5_GASTRO_DISCOUNTS_CLOSING.md), [`V3_3_STAGE_6_QR_STUDIO_CLOSING.md`](../audits/V3_3_STAGE_6_QR_STUDIO_CLOSING.md), [`V3_3_STAGE_7_ACTIVITY_COUPONS_CLOSING.md`](../audits/V3_3_STAGE_7_ACTIVITY_COUPONS_CLOSING.md), [`V3_3_STAGE_8_CAMPAIGNS_CLOSING.md`](../audits/V3_3_STAGE_8_CAMPAIGNS_CLOSING.md), [`V3_3_STAGE_9_SETTLEMENTS_CLOSING.md`](../audits/V3_3_STAGE_9_SETTLEMENTS_CLOSING.md), checklist [`Yo_Te_Invito_Checklist_V3_3_Funcional_Operativa.md`](../dev/Yo_Te_Invito_Checklist_V3_3_Funcional_Operativa.md). |
...
| **7 — Actividades + Cupones QR** | Código implementado + scanner scope hardening; DB smoke / Scanner DB / QA global pendientes | [`V3_3_STAGE_7_ACTIVITY_COUPONS_AUDIT.md`](../audits/V3_3_STAGE_7_ACTIVITY_COUPONS_AUDIT.md), [`V3_3_STAGE_7_ACTIVITY_COUPONS_CLOSING.md`](../audits/V3_3_STAGE_7_ACTIVITY_COUPONS_CLOSING.md) |
| **8 — Campañas Email / WhatsApp** | Código implementado + hardening lifecycle; DB smoke / Redis / SMTP live / QA global pendientes | [`V3_3_STAGE_8_CAMPAIGNS_AUDIT.md`](../audits/V3_3_STAGE_8_CAMPAIGNS_AUDIT.md), [`V3_3_STAGE_8_CAMPAIGNS_CLOSING.md`](../audits/V3_3_STAGE_8_CAMPAIGNS_CLOSING.md) |
| **9 — Liquidaciones, Transferencias y Canjes** | Código implementado + hardening; migration apply / DB integration / concurrency / QA global pendientes | [`V3_3_STAGE_9_SETTLEMENTS_AUDIT.md`](../audits/V3_3_STAGE_9_SETTLEMENTS_AUDIT.md), [`V3_3_STAGE_9_SETTLEMENTS_CLOSING.md`](../audits/V3_3_STAGE_9_SETTLEMENTS_CLOSING.md) |
...
**Etapa 7 — resumen:** cupones QR de **Actividades** (`Event.category = excursion`; rutas `/excursiones/*`). Dominio **vertical-specific** (`ActivityCoupon` ≠ `GastroDiscount`; no hay `GenericCoupon`). Primitives compartidos (QR payload, short-code, benefit formatter, calendario, KPIs). Claim → token + QR `yti:activity-coupon:v1:<couponId>:<token>` + short code `XXX-XXX` en `ActivityCouponClaim`. Scanner `EXCURSION_OPERATOR` **operator-wide** (`canScannerAccessActivityCoupon`). Public UX + `/me/descuentos` (bloque Actividades). Metrics + EMAIL/IN_APP de claim. Admin-only V1. Custom QR Studio Activity **diferido**. Migraciones `20260831160000_activity_coupon_domain` + `20260831170000_activity_coupon_claimed_notification` — **smoke DB pendiente**. Hardening `a494274`.

**Etapa 8 — resumen:** **transactional ≠ marketing**. Consent `UserMarketingPreference` (EMAIL/WA independientes; sin fila = no elegible; usuarios existentes sin opt-in por defecto). Unsubscribe token 64 hex; `GET /public/marketing/unsubscribe` read-only; `POST` mutación idempotente; web `/baja-promos?token=...`. Campañas `AdminCampaign` + `AdminCampaignDelivery`; cola BullMQ `campaign-emails` (separada de `emails`); worker revalida consent/user/content/cancel; template `ADMIN_CAMPAIGN` sin tracking. WhatsApp **provider-ready / envío real NOT CONFIGURED** (`canSendWhatsAppCampaign()` → false). Digest operativo vencidos `AdminExpiredBenefitsDigestService` — cron **08:20 AR** (`America/Argentina/Buenos_Aires`); no requiere opt-in marketing. Migraciones `20260831180000_user_marketing_preference`, `20260831190000_admin_campaign_domain`, `20260831200000_admin_operational_digest_log` — **smoke DB pendiente**. Hardening `39a8a0b`.

**Etapa 9 — resumen:** **NO payment gateway / NO bank API / NO fiscal invoicing**. Modelo: uso validado → `BenefitCommercialAgreement` histórico → liquidación mensual `BenefitSettlement` (`periodKey` YYYY-MM AR) → allocation `CASH` o `BARTER` por validation concreta (`@@unique([validationSource, validationId])`). CASH: `cashDueCents` / transferencias manuales `BenefitSettlementTransfer` / `CLOSED ≠ PAID`. BARTER: snapshot multiplier → `CourtesyCreditLedgerEntry` append-only (`sourceAllocationId @unique`) → funding cortesía Gastro (`DEBIT_COURTESY`, `sourceCourtesyCampaignId @unique`, ADMIN only). Reporting read-only (monthly, partners, integrity, settlement audit). Money: Prisma `BigInt` cents, API `string`, `Decimal` multiplier, half-up. Activity: crédito visible, consumo cortesía **diferido**. Hardening 9.8: deep delete guards, orphan/partner integrity, allocate `Serializable`+`FOR UPDATE`. Migraciones `20260901120000`…`20260901200000` — **apply NO EJECUTADO** (P1001). Hardening `b920553`.
...
| **`docs/audits/V3_3_STAGE_7_ACTIVITY_COUPONS_AUDIT.md`** | Auditoría Etapa 7 — Actividades + Cupones QR |
| **`docs/audits/V3_3_STAGE_7_ACTIVITY_COUPONS_CLOSING.md`** | Cierre Etapa 7 V3.3 — Actividades + Cupones QR |
| **`docs/audits/V3_3_STAGE_8_CAMPAIGNS_AUDIT.md`** | Auditoría Etapa 8 — Campañas Email / WhatsApp |
| **`docs/audits/V3_3_STAGE_8_CAMPAIGNS_CLOSING.md`** | Cierre Etapa 8 V3.3 — Campañas Email / WhatsApp |
| **`docs/audits/V3_3_STAGE_9_SETTLEMENTS_AUDIT.md`** | Auditoría Etapa 9 — Liquidaciones, Transferencias y Canjes |
| **`docs/audits/V3_3_STAGE_9_SETTLEMENTS_CLOSING.md`** | Cierre Etapa 9 V3.3 — Liquidaciones, Transferencias y Canjes |
