...
**Google Cloud:** buckets `yti-prod-storage` (privado) + `yti-prod-public-assets` (público). **Backups:** cerrados — [`GCS_BACKUPS_RUNBOOK.md`](../deploy/GCS_BACKUPS_RUNBOOK.md). **Upload:** API + Admin Rentals GCS. **`next/image`:** remotePatterns bucket público — [`GCS_STORAGE_STRATEGY.md`](../deploy/GCS_STORAGE_STRATEGY.md) §11–15. Pendiente: otros verticales.
